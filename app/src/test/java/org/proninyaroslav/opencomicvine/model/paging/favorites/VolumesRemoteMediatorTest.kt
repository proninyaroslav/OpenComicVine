...
class VolumesRemoteMediatorTest {
    ...
    fun setUp() {
        ...
        every { pref.favoriteVolumesSort } returns flowOf(sort)
    ...
    fun `Load success result when more data is present`() = runTest {
        ...
        val favoritesList = volumesList.mapIndexed { index, it ->
        ...
        val pagingState = PagingState<Int, PagingFavoritesVolumeItem>(
            pages = listOf(),
            anchorPosition = null,
            config = PagingConfig(
                pageSize = pageSize
        ...
        coVerify { volumeItemRepo.deleteByIdList(emptyList()) }
        verify { pref.favoriteVolumesSort }
        verify {
            favoritesRepo.observeByType(
                entityType = FavoriteInfo.EntityType.Volume,
                sort = sort,
    ...
    fun `Load success result and endOfPaginationReached when no more data`() = runTest {
        ...
        val itemsList = emptyList<PagingFavoritesVolumeItem>()
        val pagingState = PagingState<Int, PagingFavoritesVolumeItem>(
            pages = listOf(),
            anchorPosition = null,
            config = PagingConfig(
    ...
    fun `Service error`() = runTest {
        ...
        confirmVerified(volumesRepo, favoritesRepo, pref, response)
    }
}